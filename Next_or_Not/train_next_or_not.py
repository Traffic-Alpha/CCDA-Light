...
LastEditTime: 2024-06-26 23:32:32
'''
import os
import torch
import argparse
...
def train_model(env, delta_time, tensorboard_path, callback_list):
    ...
    model = PPO(
        ...
        env, 
        batch_size=128, 
        n_steps=7200//delta_time, # 每次更新的样本数量为 n_steps*NUM_CPUS, n_steps 太小可能会收敛到局部最优, 推荐大小是 n_steps*NUM_CPUS//2
    ...
    model.learn(total_timesteps=1e6, tb_log_name=f'NextorNot_{delta_time}', callback=callback_list)
    return model


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Process delta_time.')
    parser.add_argument('--delta_time', type=int, default=5, help='The delta time value')
