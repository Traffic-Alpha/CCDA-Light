'''
@Author: WANG Maonan
@Date: 2022-06-17 16:48:07
@Description: Choose Phase Wrapper
@LastEditTime: 2022-08-16 18:21:10
'''
import gym
import numpy as np
...
class env_wrapper(gym.Wrapper):
    def __init__(
        ...
        self.is_shuffle = is_shuffle # 是否打乱 obs (进行行乱序)
        # 训练或是测试模式, reset 有所区别
    ...
    def reset(self):
        ...
        if (self.mode == 'train') or (self.reset_num == 0):
            if self.route_files is not None:
                ...
                observations = self.env.reset(route_file=route_file)
            else: # 不随机 route 文件
    ...
    def _process_obs(self, observation):
        ...
        # 对 obs 进行 shuffle
        if self.is_shuffle:
            np.random.shuffle(_observation_net_info) # 直接对原始 array 进行修改
