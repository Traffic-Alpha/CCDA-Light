...
@Description: Choose Next Phase 模型
- nohup srun -J ChooseNextPhase -p CPU -n 1 -c 30 -w st-node-159 python train.py > node_159.log &
srun -J ChooseNextPhase -p CPU -n 1 -c 30 -w st-node-159 python train.py --net_folder=fourWay --net_id=4phases --route_id=fluctuation --delta_times=10
@LastEditTime: 2022-08-17 17:43:26
'''
import os
import argparse
...
if __name__ == '__main__':
    ...
    EVAL_FREQ = EPISODE_STEP*2 # 测试的频率, 完成两次仿真保存一次
    ...
    # 保存 checkpoint, env 部分
    checkpoint_callback = CheckpointCallback(
        save_freq=SAVE_FREQ,
        save_path=MODEL_PATH,
    )
    # 保存 checkpoint, vec norm 部分
