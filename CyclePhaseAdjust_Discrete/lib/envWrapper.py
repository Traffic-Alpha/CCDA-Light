...
@LastEditTime: 2022-10-08 10:50:14
'''
import itertools
import logging
import gym
...
class env_wrapper(gym.Wrapper):
    def __init__(
        ...
        _actions = [
            ...
            ] # 每个相位可以的取值, 可以组成 [[-5, 0, 5], [-5, 0, 5]]
        self.actions = sorted(set(itertools.product(*_actions, repeat=1))) # 组合出所有的可能, 需要确保每次顺序是一样的
    ...
    def step(self, action):
        ...
        例如是二相位, 每个相位可以调节的时间为 [[-5, 0, 5], [-5, 0, 5]], 
        于是一共有 9 种不同的可能性, 所以动作空间是 9. 下面是 9 种可能:
        [
            (-5, -5), 
            (-5, 0), 
            (-5, 5), 
