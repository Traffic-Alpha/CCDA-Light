...
LastEditTime: 2022-08-16 22:58:00
'''
...
class env_wrapper(gym.Wrapper):
    def __init__(
            ...
            duration_range=[[-6,6,2],[-6,6,2],[-6,6,2],[-6,6,2]], # 自定义每一个 phase 的变化范围
        ...
        self.actions = [
                np.arange(duration_range[i][0], duration_range[i][1]+1, duration_range[i][2]) 
                for i in range(len(duration_range))
            ] # 每个相位可以的取值
        self.action_space = spaces.MultiDiscrete([len(_phase_action) for _phase_action in self.actions])
    ...
    def step(self, action):
        """这里 action 为与相位数量相同的列表, 下面看一个详细的例子:
        1. 现在是四相位, 初始的时间为 [30, 30, 30, 30]
        2. action 为 [0,2,1,4]
        3. 每一个 phase 可以进行修改的时间为:
            [
                array([-6, -4, -2,  0,  2,  4,  6]),
                array([-6, -4, -2,  0,  2,  4,  6]),
                array([-6, -4, -2,  0,  2,  4,  6]),
        ...
        Args:
            action (int): action 为与相位数量相同的列表
        """
        _action = [
            self.actions[phase_index][action_index] 
            for phase_index, action_index in enumerate(action)
        ] # 得到每一个相位要修改的时间
