'''
@Author: WANG Maonan
@Date: 2022-08-16 20:08:09
@Description: 一个周期同时调整每个相位的时间
- nohup srun -J CyclePhaseAdjust_MultiDiscrete -p CPU -n 1 -c 30 -w st-node-159 python train.py > node_159.log &
@LastEditTime: 2022-11-08 12:22:00
'''
import os
import argparse
...
from lib import makeENV, models, netConfig
...
if __name__ == '__main__':
    ...
    # #######
    NUM_CPUS = 30
    EPISODE_STEP = int(7000/70) if delta_times==None else int(7000/delta_times) # 每一次仿真的交互次数, 7000/delta time (总的仿真时间/每一步的时间)
    EVAL_FREQ = EPISODE_STEP*2 # 测试模型的频率
    ...
    DURATION_RANGE = [
        [-10, 10, 5] for _ in range(int(net_id[0])) # net_id=6phases, 第一个数字表明路网有多少相位
    ] # 每个 phase 变化范围
    ...
    params = {
        ...
        'log_file':LOG_PATH,
        'duration_range': DURATION_RANGE # 相位调整的变化, 用来生成初始化 action 的
    }
    ...
    model.learn(
        total_timesteps=1e6,
        tb_log_name=f'{delta_times}_{net_folder}_{net_id}_{route_id}', 
